...
class ModifiedImage{
	...
	{
            $this->_ruta_destino="images/p_images/";
	...
	{
		if($this->_imageType == IMAGETYPE_JPEG){
			imagejpeg($this->_image, $this->_ruta_destino.$fileName, $compression);
		}
		elseif($this->_imageType == IMAGETYPE_GIF){
			imagegif($this->_image, $this->_ruta_destino.$fileName);
		}
		elseif($this->_imageType == IMAGETYPE_PNG){
			imagepng($this->_image, $this->_ruta_destino.$fileName);
		}
	...
	public function resizeToFit($width, $height, $margins = false, $hexBckColor = '000000') {
		...
		if($newW != $width || $newH != $height) {
			$newImage = imagecreatetruecolor($width, $height);
                        if($hexBckColor==="alpha" && $this->getImageType()==="image/png")
                        {
                            $color = imagecolorallocatealpha($newImage, 0, 0, 0, 127);
                            imagefill($newImage, 0, 0,$color);
                            imagesavealpha($newImage, TRUE); // para que se suba como alpha
                        }
                        else
                        {
